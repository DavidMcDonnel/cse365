		...
		try {
			...
			for (String filename : files) {
				...
				while(parser.hasMoreCommands()){
					...
					}else if(commandType.equals(VMCommandType.C_LABEL)){
						w.writeLabel(parser.currentCommand+"$"+parser.arg1());
					}else if(commandType.equals(VMCommandType.C_IF)){
						w.writeIf(parser.currentCommand+"$"+parser.arg1());
					}else if(commandType.equals(VMCommandType.C_GOTO)){
						w.writeGoto(parser.currentCommand+"$"+parser.arg1());
					}else if(commandType.equals(VMCommandType.C_FUNCTION)){
						w.writeFunction(parser.arg1(),parser.arg2());
					}else if(commandType.equals(VMCommandType.C_RETURN)){
						w.writeReturn();
					}else if(commandType.equals(VMCommandType.C_CALL)){
