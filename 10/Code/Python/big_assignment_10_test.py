...
def test(filename):
    ...
    c = JackParser(filename)
    n = c.parse_class()
    p_xml = str(n).strip()
    
    p_xml_ref = read_whole_file(folder + os.sep + core + '.xml')  
    dump_to_file(folder + os.sep + "my" + core + '.xml', p_xml)    
    print '\tParsed Correctly?   ', p_xml == p_xml_ref
